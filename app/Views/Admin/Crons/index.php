...
<div class="row g-4 mt-4">
    <div class="col-lg-12">
        <div class="card settings-card">
            <div class="card-body p-4">
                <h5 class="fw-bold mb-3" style="color: var(--primary);"><i class="fa-solid fa-clock me-2"></i> Automated Backups</h5>
                <p class="text-muted small mb-3">Configure a cron job to create regular database backups automatically.</p>
                <form id="backupForm" class="row g-3 mb-4">
                    <div class="col-md-6">
                    ...
                    <div class="col-12">
                        <label class="form-label fw-semibold">Exclude tables</label>
                        <select class="form-select" name="exclude_tables[]" id="excludeTables" multiple size="5">
                            <option value="ci_sessions">ci_sessions (sessions)</option>
                            <option value="tbl_Audit_Log">tbl_Audit_Log (audit trail)</option>
                            <option value="tbl_Cron_Log">tbl_Cron_Log (cron history)</option>
                            <option value="tbl_Email_Log">tbl_Email_Log (email log)</option>
                            <option value="tbl_Sms_Processing">tbl_Sms_Processing</option>
                            <option value="tbl_Analyzed_Transactions">tbl_Analyzed_Transactions</option>
                            <option value="tbl_Transaction_Tag_Map">tbl_Transaction_Tag_Map</option>
                        </select>
                        <small class="text-muted">Hold Ctrl/Cmd to select multiple. Large log tables are excluded by default for scheduled backups.</small>
                    </div>
                    <div class="col-12">
                        <button type="submit" class="btn btn-primary btn-lg rounded-pill px-5 fw-semibold">
                            <i class="fa-solid fa-download me-1"></i> Download Backup Now
                        </button>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <h6 class="fw-bold">Cron URL for Scheduled Backups</h6>
                        <code class="d-block bg-light p-2 rounded small text-break"><?= base_url('admin/backup/scheduled') ?>?key=YOUR_SECRET_KEY</code>
                    </div>
                    <div class="col-md-6">
                        <h6 class="fw-bold">Setup Instructions</h6>
                        <ol class="small text-muted mb-0">
                            <li>Set <code>BACKUP_CRON_KEY</code> in <code>.env</code> (or environment variable)</li>
                            <li>Add cron entry: <code>0 2 * * * curl -s "URL" >/dev/null</code> (runs daily at 2 AM)</li>
                            <li>Backups saved to <code>writable/backups/</code> (keeps last 7)</li>
                        </ol>
                    </div>
                </div>
            </div>
...
// Automated Backups section (moved from /admin/backup)
    ...
    document.querySelectorAll('.btn-delete-backup').forEach(btn => {
        btn.addEventListener('click', async function() {
            ...
            const result = await Swal.fire({
                title: 'Delete Backup?',
                text: `This will permanently delete "${file}".\n\nThis action CANNOT be undone.`,
                icon: 'warning',
